...
- [ ] **M7 — 문항 파서** (question-spec 2-1)
  - [ ] `api/_question.py`: 마커 분리/대상 나열 판별/복동사 분해/인라인 나열/괄호·개수 추출
  - [ ] LLM 폴백 1콜 (요구 text 원문 부분 문자열 검증, 키 없으면 simple 강등)
  - [ ] 스키마 v1.1: `procedure` 부품 추가 + build_index 검증 확장
  - [ ] `tests/test_question.py`: 부록 A 표본 24문 픽스처, 규칙 경로 ≥22/24
- [ ] **M8 — 요구 기반 조립** (question-spec 2-2, 2-4, 2-5)
  - [ ] `_assembler.py`: 요구→단락 가변(Ⅱ~Ⅵ), 동사·트리거→부품 슬롯 매핑, 대체 순서, 압축/확장 렌더
  - [ ] `_volume` 재배분 (N=1~2: 1쪽 / 3: 0.7쪽 / 4~5: 0.5쪽, 요구당 최소 7줄)
  - [ ] 부족 슬롯 일괄 집필 1콜 — 파서 폴백·모호 매칭 합산 총 2콜 상한 유지
- [ ] **M9 — 동적 로드맵 서론** (question-spec 2-3)
  - [ ] d7 로드맵 생성기: d-box = 요구 라벨(12자 절삭), 박스 텍스트 ⊂ 해당 h2 제목
  - [ ] 서론 타입 선택 규칙 (요구 2+ → Type IV / 0~1 → Type I / 보안 → Type III)
  - [ ] 세아 검증 확장: 박스-h2 일치, 요구 순서, 비교→tcmp·절차→procedure 존재
  - [ ] `answer-template-spec.md` §5 개정 (다인 협의 — 단락 수 가변 명시)
- [ ] **M10 — QA·풀부품 보강**
  - [ ] question-spec 3장 완료 기준 curl 전항목 (Q1~Q5 + 부족 슬롯 + 회귀)
  - [ ] 절차성 풀부품에 procedure 보강 (ISP·ISMS·DRS 우선), CLAUDE.md 갱신
