...
5인 AI 에이전트 팀이 서로 대화하며 **진짜 동작하는 웹앱을 만들어주는** 메이커 서비스입니다. 팀 보드에서 누가 무엇을 하는지 실시간으로 보입니다. "테트리스 만들어줘" 하면 기획→디자인→개발→QA를 거쳐 미리보기/다운로드 가능한 HTML 작업물이 나옵니다. **Vercel에 무료로 배포**할 수 있고, **무료 LLM**(Groq / Gemini)으로 동작합니다.
...
에이전트들의 상태와 **서로 주고받는 대화**가 채팅 응답 스트림(NDJSON)에 실려 브라우저로 전달되고, 팀 보드의 카드가 실시간으로 상태·대사·파이프라인 진행도를 표시합니다. 완성된 작업물은 미리보기 모달과 다운로드 버튼으로 제공됩니다.
...
api/_static/
  index.html  # 레이아웃 (팀 보드 + 채팅 패널)
  office.js   # 팀 보드 렌더러 (에이전트 카드/상태 칩/말풍선/진행선)
  app.js      # 채팅 UI + NDJSON 스트림 파싱 + 작업물 미리보기/다운로드
  style.css   # 모던 라이트 테마 (Pretendard)
